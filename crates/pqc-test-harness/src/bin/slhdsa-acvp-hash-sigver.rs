//! Execute supported NIST ACVP FIPS 205 external HashSLH-DSA SigVer vectors.
...
use pqc_slh_dsa::{SlhDsa, SlhDsaParameterSet, SlhDsaPreHash, SlhDsaPublicKey, SlhDsaSignature};
...
fn main() {
    if let Err(error) = run() {
        eprintln!("SLH-DSA ACVP external-prehash SigVer failed: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    println!("NIST ACVP FIPS 205 external HashSLH-DSA SigVer");
...
fn verify_case(
    ...
    let hash_alg = hash_alg.ok_or_else(|| "prehash case omitted hashAlg".to_owned())?;
