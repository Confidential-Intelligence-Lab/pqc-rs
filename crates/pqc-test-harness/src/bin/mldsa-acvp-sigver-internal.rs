...
fn main() {
    if let Err(e) = run() {
        eprintln!("ML-DSA ACVP internal sigVer failed: {e}");
        std::process::exit(1)
    }
}
fn run() -> Result<(), String> {
    ...
    if a.len() != 4 {
        return Err("usage: sigver prompt response expected".to_owned());
    }
    ...
    let (m, x) = compare(&r, &e)?;
    println!("NIST ACVP ML-DSA sigVer internal response");
...
fn process(p: &Value) -> Result<(Value, usize, usize, usize, usize, usize), String> {
    ...
    for gv in groups {
        ...
        for tv in g.get("tests").and_then(Value::as_array).ok_or("tests")? {
            ...
            let pk = hex_decode(req_str(t, "pk")?)?;
            let sig = hex_decode(req_str(t, "signature")?)?;
            let ok = if em {
                let mu = fixed::<64>(req_str(t, "mu")?)?;
                verify_internal_mu(ps, &pk, &mu, &sig)
            } else {
                let msg = hex_decode(req_str(t, "message")?)?;
                verify_internal_message(ps, &pk, &msg, &sig)
            }
            .unwrap_or(false);
            tests.push(json!({"tcId":id,"testPassed":ok}));
            tc += 1
        }
...
fn compare(a: &Value, e: &Value) -> Result<(usize, usize), String> {
    ...
    for (k, v) in aa {
        ...
        if req_bool(v, "testPassed")? == req_bool(q, "testPassed")? {
            m += 1
        } else {
            x += 1
        }
