//! Execute supported NIST ACVP FIPS 205 external HashSLH-DSA SigGen vectors.
...
use pqc_slh_dsa::{SlhDsa, SlhDsaParameterSet, SlhDsaPreHash, SlhDsaPrivateKey};
...
fn main() {
    if let Err(error) = run() {
        eprintln!("SLH-DSA ACVP external-prehash SigGen failed: {error}");
        std::process::exit(1);
    }
}
...
fn run() -> Result<(), String> {
    ...
    let tg_filter = arguments
        .get(2)
        .map(|value| {
            value
                .parse::<u64>()
                .map_err(|error| format!("invalid tgId filter: {error}"))
        })
        .transpose()?;
    ...
    println!("NIST ACVP FIPS 205 external HashSLH-DSA SigGen");
...
fn execute_case(
    ...
    let hash_alg = hash_alg
        .ok_or_else(|| format!("tgId={tg_id} tcId={tc_id}: prehash case omitted hashAlg"))?;
    let prehash = SlhDsaPreHash::from_acvp_name(hash_alg)
        .ok_or_else(|| format!("tgId={tg_id} tcId={tc_id}: unsupported hashAlg {hash_alg}"))?;
    ...
    let signature = if deterministic {
        ...
        implementation
            .hash_sign_deterministic(&private_key, &message, &context, prehash)
            .map_err(|error| {
                format!(
        ...
        implementation
            .hash_sign_hedged(&private_key, &message, &context, prehash, &mut rng)
