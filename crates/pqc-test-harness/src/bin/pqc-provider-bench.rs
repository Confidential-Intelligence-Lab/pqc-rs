use pqc_ml_dsa::{
    keygen::keygen_internal as dsa_keygen,
    signature::sign_internal as dsa_sign,
    verification::verify_internal as dsa_verify,
    MlDsaParameterSet,
};
use pqc_ml_kem::{
    ml_kem_decaps::decaps_internal,
    ml_kem_encaps::encaps_internal,
    ml_kem_keygen::ml_kem_768_keygen_internal,
    MlKemParameterSet,
};
use serde_json::json;
...
const MESSAGE: &[u8] =
    b"pqc-rfc9958-rs B1.3.5 performance baseline";
...
impl Bench {
    fn new() -> Self {
        let kem_keypair =
            ml_kem_768_keygen_internal(&KEM_D, &KEM_Z)
                .expect("ML-KEM setup keygen failed");
        ...
        let dsa_keypair =
            ...
            .expect("ML-DSA setup keygen failed");
        ...
        Self {
            kem_public_key:
                kem_keypair.encapsulation_key.to_vec(),
            kem_private_key:
                kem_keypair.decapsulation_key.to_vec(),
            kem_ciphertext:
                kem_enc.ciphertext.clone(),

            dsa_public_key:
                dsa_keypair.public_key().to_vec(),
            dsa_private_key:
                dsa_keypair.private_key().to_vec(),
            dsa_signature,
...
fn main() {
    ...
    match operation.as_str() {
        "kem-keygen" => benchmark(
            "ML-KEM-768",
            "kem-keygen",
            &mut bench,
            Bench::kem_keygen,
        ),
        "kem-encaps" => benchmark(
            "ML-KEM-768",
            "kem-encaps",
            &mut bench,
            Bench::kem_encaps,
        ),
        "kem-decaps" => benchmark(
            "ML-KEM-768",
            "kem-decaps",
            &mut bench,
            Bench::kem_decaps,
        ),
        "dsa-keygen" => benchmark(
            "ML-DSA-65",
            "dsa-keygen",
            &mut bench,
            Bench::dsa_keygen,
