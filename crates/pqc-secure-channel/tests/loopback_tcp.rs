...
const APPLICATION_CONTEXT: &[u8] = b"pqc-forge-e5-loopback-tcp";
const REQUEST_AAD: &[u8] = b"pqc-forge-e5-request";
const RESPONSE_AAD: &[u8] = b"pqc-forge-e5-response";

const REQUEST: [u8; 1024] = [0xa5; 1024];
const RESPONSE: [u8; 1024] = [0x5a; 1024];

const TCP_TRANSFER_LIMIT: usize = 7;
...
struct TransportStats {
    ...
    max_transmit_progress: usize,
    max_receive_progress: usize,
}
...
impl TransportTransmit for TcpTransport {
    fn transmit(&mut self, input: &[u8]) -> TransportResult<usize> {
        if input.is_empty() {
            return Ok(0);
        }

        let limit = input.len().min(TCP_TRANSFER_LIMIT);

        match self.stream.write(&input[..limit]) {
            Ok(0) => Err(TransportError::Closed),
            Ok(progress) => {
                self.stats.transmit_calls += 1;
                self.stats.transmitted_bytes += progress;
                self.stats.max_transmit_progress = self.stats.max_transmit_progress.max(progress);
...
impl TransportReceive for TcpTransport {
    fn receive(&mut self, output: &mut [u8]) -> TransportResult<usize> {
        ...
        let limit = output.len().min(TCP_TRANSFER_LIMIT);

        match self.stream.read(&mut output[..limit]) {
            Ok(0) => Err(TransportError::Closed),
            Ok(progress) => {
                self.stats.receive_calls += 1;
                self.stats.received_bytes += progress;
                self.stats.max_receive_progress = self.stats.max_receive_progress.max(progress);
...
fn send_frame(transport: &mut TcpTransport, frame: &ProtocolFrame<'_>) {
...
fn receive_frame(transport: &mut TcpTransport) -> Vec<u8> {
...
fn exercise_profile(fixture: ProfileFixture) {
    ...
    assert!(client_stats.transmit_calls > 1);
    assert!(client_stats.receive_calls > 1);
    assert!(server_observation.transport.transmit_calls > 1);
    assert!(server_observation.transport.receive_calls > 1);

    assert!(client_stats.max_transmit_progress <= TCP_TRANSFER_LIMIT);
    assert!(client_stats.max_receive_progress <= TCP_TRANSFER_LIMIT);
    assert!(server_observation.transport.max_transmit_progress <= TCP_TRANSFER_LIMIT);
    assert!(server_observation.transport.max_receive_progress <= TCP_TRANSFER_LIMIT);

    assert_eq!(sender.sequence_number(), 1);
    assert_eq!(receiver.sequence_number(), 1);

    println!(
        "E5|{}|capability={:#06x}|tcp=loopback|transfer_limit={}|\
client_tx_calls={}|client_rx_calls={}|server_tx_calls={}|server_rx_calls={}|\
client_tx_bytes={}|client_rx_bytes={}|server_tx_bytes={}|server_rx_bytes={}|\
request_plaintext_bytes={}|response_plaintext_bytes={}|\
client_sender_sequence={}|client_receiver_sequence={}|\
server_sender_sequence={}|server_receiver_sequence={}|PASS",
        fixture.label,
        fixture.capability.value(),
        TCP_TRANSFER_LIMIT,
        client_stats.transmit_calls,
        client_stats.receive_calls,
        server_observation.transport.transmit_calls,
        server_observation.transport.receive_calls,
        client_stats.transmitted_bytes,
        client_stats.received_bytes,
        server_observation.transport.transmitted_bytes,
        server_observation.transport.received_bytes,
        REQUEST.len(),
        RESPONSE.len(),
...
#[test]
fn loopback_tcp_succeeds_for_all_registered_secure_channel_profiles() {
    for fixture in fixtures() {
        exercise_profile(fixture);
    }
}
