...
static int sig(const char *op,const char *alg,int argc,char **argv){OQS_SIG *s=OQS_SIG_new(sig_alg_name(alg));if(!s)return 2;int rc=1;
 ...
 else if((!strcmp(op,"dsa-sign")||!strcmp(op,"slh-sign"))&&argc>=6){size_t sn,mn,cn;unsigned char *sk=from_hex(argv[3],&sn),*m=from_hex(argv[4],&mn),*ctx=from_hex(argv[5],&cn),*out=malloc(s->length_signature);size_t outn=0;if(sk&&m&&ctx&&sn==s->length_secret_key&&OQS_SIG_sign_with_ctx_str(s,out,&outn,m,mn,ctx,cn,sk)==OQS_SUCCESS){print_hex("signature",out,outn);rc=0;}free(sk);free(m);free(ctx);free(out);}
 else if((!strcmp(op,"dsa-verify")||!strcmp(op,"slh-verify"))&&argc>=7){size_t pn,mn,cn,gn;unsigned char *pk=from_hex(argv[3],&pn),*m=from_hex(argv[4],&mn),*ctx=from_hex(argv[5],&cn),*g=from_hex(argv[6],&gn);if(pk&&m&&ctx&&g&&pn==s->length_public_key){printf("valid=%s\n",OQS_SIG_verify_with_ctx_str(s,m,mn,g,gn,ctx,cn,pk)==OQS_SUCCESS?"true":"false");rc=0;}free(pk);free(m);free(ctx);free(g);}
